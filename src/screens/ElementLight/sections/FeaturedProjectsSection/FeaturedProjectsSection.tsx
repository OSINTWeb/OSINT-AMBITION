...
const projectsData = [
  {
    ...
    image: "/project/profiler.me.jpg",
    gridClass: "col-span-1",
    url: "https://profiler.me/",
    external: true,
  },
  {
    id: 2,
  ...
  },
  {
    ...
    gridClass: "col-span-1",
    url: "https://forum.osintambition.org",
    external: true,
  },
];
...
export const FeaturedProjectsSection = (): JSX.Element => {
  return (
    <section className="w-full relative">
      <div className="max-w-[1380px] mx-auto px-[30px] max-[480px]:px-4">
        ...
        <div className="grid grid-cols-2 max-[480px]:grid-cols-1 gap-[30px] max-[480px]:gap-4 auto-rows-max">
          {projectsData.map((project) => {
            const anchorProps = project.external
              ? { href: project.url, target: "_blank", rel: "noopener noreferrer" }
              : { href: project.url };
            return (
              <Card
                key={project.id}
                className={`${project.gridClass} max-[480px]:col-span-1 bg-transparent border-none overflow-hidden group cursor-pointer`}
              >
                <a {...anchorProps} className="block no-underline">
                  <CardContent className="p-0 relative">
                    <div
                      className={`w-full bg-contain bg-center bg-no-repeat aspect-[16/9] max-[480px]:aspect-[4/3]`}
